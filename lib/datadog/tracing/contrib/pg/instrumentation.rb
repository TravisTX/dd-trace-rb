          ...
          module InstanceMethods
            def exec(sql)
              ...
              Tracing.trace(Ext::SPAN_EXEC, service: service) do |span|
                ...
                span.kind = Tracing::Metadata::Ext::Kind::CLIENT
                span.service = Ext::DEFAULT_PEER_SERVICE_NAME

                span.set_tag('db.instance', self.db)
                span.set_tag('db.user', self.user)
                span.set_tag('db.system', Ext::SPAN_SYSTEM)
                span.set_tag('db.statement', sql)

                span.set_tag('network.destination.name', self.host)
                span.set_tag('network.destination.port', self.port)

                span.set_tag(Tracing::Metadata::Ext::TAG_COMPONENT, Ext::TAG_COMPONENT)
                span.set_tag(Tracing::Metadata::Ext::TAG_OPERATION, Ext::TAG_OPERATION_QUERY)

                span.set_tag(Ext::TAG_DB_NAME, self.db)

                # Tag as an external peer service
                span.set_tag(Tracing::Metadata::Ext::TAG_PEER_SERVICE, service)
                span.set_tag(Tracing::Metadata::Ext::TAG_PEER_HOSTNAME, self.host)
                span.set_tag(Tracing::Metadata::Ext::NET::TAG_TARGET_HOST, self.host)
                span.set_tag(Tracing::Metadata::Ext::NET::TAG_TARGET_PORT, self.port)
